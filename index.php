<?php
// public/index.php (revisado – apenas exibição/UI)
session_start();
...
// agrupa por entidade para a lista visual (ignorando “traço”)
$groupFiltered=[];
if($logged && !empty($data['entities'])){
  foreach($data['entities'] as $e){
    if(isset($e['name']) && trim($e['name'])==='-') continue; // remove agrupador geral
    ...
    foreach($e['items'] as $it){
      ...
      $bucket['items'][]=$it;
      $bucket['total']+=(float)$it['value'];
      $bucket['received']+=(float)$it['received'];
    }
    if($bucket['items']) $groupFiltered[]=$bucket;
  }
}
$FIRST_GROUPS = 9999; // sempre mostrar tudo aberto (sem "ver mais")
...
<?php else: ?>
  ...
  <!-- DRAWER (um modal com subtelas + Voltar) -->
...
(function(){
  ...
  function openDrawer(title, items, kind, opts){
    ...
    if(kind==='entity'){
      ...
      // lista do mês (clicado no gráfico)
      var byDay = {};
      norm.forEach(function(i){
      ...
      // lista do dia (clicado em "Próximas datas")
      ...
      // recebidos recentes (por mês)
      var grouped=Object.keys(byMonth).filter(function(k){return k!=='sem-data';}).sort(function(a,b){return b.localeCompare(a);}).slice(0,6).map(function(ym){return [ym,byMonth[ym]];});
      dynamicHtml='<div class="drawer__section"><header class="drawer__section-head"><h4>Recebimentos recentes</h4></header><div class="drawer__list">'
        + grouped.map(function(kv){
            var ym=kv[0], stat=kv[1], p=ym.split('-'); var year=p[0], month=Number(p[1]||'1')-1;
            return '<div class="info-line" data-tip="'+(stat.count||0)+' item(ns)"><div><strong>'+['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'][month]+'/'+year.slice(-2)+'</strong></div><span class="tag">'+formatBRL(stat.sum)+'</span></div>';
          }).join('')
        + '</div></div>';
      dBody.innerHTML = summaryHtml + dynamicHtml;
  ...
  function bindOpeners(){
    // KPIs usam SEMPRE os itens filtrados – garante contagens alinhadas com a tela
